...
FIRESTORE_COLLECTION = os.environ.get('FIRESTORE_COLLECTION', 'blockchain-core-ohlcv-data')
GCP_PROJECT_ID = os.environ.get('GCP_PROJECT_ID')
RAW_DATA_BUCKET = os.environ.get('RAW_DATA_BUCKET') # This will be set by the deploy script
...
def process_trade_data(event, context):
    ...
    try:
        ...
        if RAW_DATA_BUCKET:
            ...
            print("RAW_DATA_BUCKET environment variable not set. Skipping raw data storage.")
...
def store_raw_in_cloud_storage(data):
    ...
    try:
        ...
        print(f"Successfully stored raw trade {data['trade_id']} in Cloud Storage bucket {RAW_DATA_BUCKET}.")
    except Exception as e:
