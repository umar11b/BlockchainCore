...
class AnomalyDetector:
    ...
    def get_recent_ohlcv_data(self, symbol: str, minutes: int = 60) -> List[Dict[str, Any]]:
    ...
    def detect_price_anomaly(self, ohlcv_data: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        if abs(price_change_pct) > self.price_threshold:
            return {
                ...
                'severity': 'high' if abs(price_change_pct) > self.price_threshold * 2 else 'medium'
            }
        
        return None
    
    def detect_volume_anomaly(self, ohlcv_data: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        if volume_ratio > self.volume_threshold:
            return {
                ...
                'severity': 'high' if volume_ratio > self.volume_threshold * 2 else 'medium'
            }
        
        return None
    
    def calculate_sma(self, ohlcv_data: List[Dict[str, Any]], period: int = 20) -> Optional[float]:
    ...
    def detect_sma_divergence(self, ohlcv_data: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        if abs(divergence_pct) > self.sma_threshold:
            return {
                ...
                'timestamp': current['timestamp'],
                'severity': 'high' if abs(divergence_pct) > self.sma_threshold * 2 else 'medium'
            }
...
Time: {anomaly['timestamp']}
            ...
            message = f"Unknown anomaly type: {anomaly}"
        
        # Send SNS message
        response = sns_client.publish(
            TopicArn=SNS_TOPIC_ARN,
            Subject=f"BlockchainCore Alert: {anomaly['type'].replace('_', ' ').title()}",
...
def lambda_handler(event, context):
    ...
    try:
        ...
        return {
            'statusCode': 200,
            'body': json.dumps({
                'message': f'Anomaly detection complete',
