...
class GCPProducer:
    ...
    def __init__(self):
        ...
        logger.info("GCP Producer initialized")
    ...
    async def process_and_send(self, trade_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.debug(
                f"Published message {message_id} for {trade_data.get('s', 'unknown')}"
            )
    ...
    def store_raw_data(self, trade_data: Dict[str, Any]):
        ...
        try:
            if not self.storage_bucket:
                logger.warning(
                    "No storage bucket configured, skipping raw data storage"
                )
                return
            ...
            filename = (
                f"raw-data/{timestamp.strftime('%Y/%m/%d/%H/%M')}/"
                f"{trade_data.get('s', 'unknown')}_{int(timestamp.timestamp())}.json"
            )
