...
if aws lambda get-function --function-name blockchaincore-api --region "$REGION" >/dev/null 2>&1; then
    ...
    aws lambda create-function \
        ...
        --environment Variables='{
            "DYNAMODB_TABLE_NAME": "'$(cd "$PROJECT_DIR/terraform" && terraform output -raw dynamodb_table_name)'",
            "S3_BUCKET_NAME": "'$(cd "$PROJECT_DIR/terraform" && terraform output -raw s3_bucket_name)'",
            "SNS_TOPIC_ARN": "'$(cd "$PROJECT_DIR/terraform" && terraform output -raw sns_topic_arn)'"
        }' \
        --region "$REGION"
fi
